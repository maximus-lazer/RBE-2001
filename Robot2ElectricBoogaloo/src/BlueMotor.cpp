...
void BlueMotor::setMotorState(motorStateChoices STATE) {
  setMotorState(STATE, false);
}

/**
 * Set the arm to move to a certain position based on state
 * @param STATE the state for the arm to move to
 * @param blocking set if code should be blocking
 */
void BlueMotor::setMotorState(motorStateChoices STATE, bool blocking) {
  switch(STATE) {
    ...
    case PRE45:
      ...
      // Blocks if blocking is true and if the motor isn't in position
      while (blocking && !isFinished()) {
        moveTo(motorTarget);
      }
      moveTo(motorTarget);
