...
// Constants
int leftLineTrackerPin = A3; // A2 pin || 20
int rightLineTrackerPin = A2; // A3 pin || 21
...
float baseTurnSpeed = 45;



enum robotState{BEGIN, PICKUPSTART, PLACESTART, CROSSED_1, DRIVESHED, PICKUP, PLACE, BACKDRIVESHED, CROSSED_2, 
                DRIVEPLATFORM, PLATFORMPLACE, ENDPICKUP, PLACETURNTOLINE, DRIVEOTHERSIDE, CROSSED_3, STOP, RESET} currentState, nextState;
...
void lineTrackWithUltraSonic(float distance) {
  ...
  // Constants
  float kP_L = 1; // LineTrack Proportional value
...
void setRobotState(robotState STATE) {
  ...
  switch(STATE) {
    ...
    case CROSSED_1:
      ...
      } else if (checkTracker || currentTime - previousTime <= 1500) {
    ...
    case DRIVESHED:
      ...
      // 3 for pickup 5 / 5.5 for pre
      // distance = 4.25; // Desired distance away from the Shed in inches
      // Line tracks with range finder for the distance specified then sets checkFinished to be true
      if (check && !withinDistance(distance)) {
        lineTrackWithUltraSonic(distance);
      } else if (autoStage) {
        check = false;
        chassis.idle();
    ...
    case BACKDRIVESHED:
      ...
      if (distanceFlag && cmToInches(rangefinder.getDistance()) < distance) {
        ...
        if (check && checkTracker || currentTime - previousTime <= 1000) {
          chassis.setMotorEfforts(baseTurnSpeed * turnDirection, -baseTurnSpeed * turnDirection);
    ...
    if (currentTime - previousTime <= 500) {
        chassis.setMotorEfforts(baseDriveSpeed, baseDriveSpeed);
      } else if (leftLineTrack < 300 || currentTime - previousTime <= 1500) {
        chassis.setMotorEfforts(-baseTurnSpeed * turnDirection, baseTurnSpeed * turnDirection);
      } else {
        currentState = STOP;
    ...
    case PLACETURNTOLINE:
      ...
      if (check && (rightLineTrack < 300 || currentTime - previousTime <= 1000)) {
    ...
    case PLACE:
      ...
      // was 4
      distance = 3; // Desired distance away from the Shed in inches
      // Line tracks with range finder for the distance specified then sets checkFinished to be true
      if (!withinDistance(distance)) {
        lineTrackWithUltraSonic(distance);
      }
    ...
    case DRIVEOTHERSIDE:
      ...
      if (check && currentEncoderCount - previousEncoderCount <= ticksPer90Deg) {
        ...
        chassis.setMotorEfforts(baseDriveSpeed, baseDriveSpeed + 5);
      } else {
        currentState = STOP;
        nextState = CROSSED_3;
        previousTime = currentTime;
...
void handleKeyPress(int keyPress)
{
  ...
  if (keyPress == remoteBack) {
    currentState = RESET;
    setRobotState(currentState);
  }

  if (keyPress == remote1) {
    currentState = PLACESTART;
    armAuto45 = false;
    autoStage = false;
    setRobotState(currentState);
  }
